...
async def add_saldo(c: Client, m: CallbackQuery):
    ...
    await m.edit_message_text(
        """<a href='https://s11.aconvert.com/convert/p3r68-cdx67/xsbmr-qgtw8.jpg'>&#8204</a><b><b>💸 Adicionar saldo</b>
...
async def add_saldo_manual(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton("📲 Enviar Comprovante", url="t.me/LittleNucky"),
			],
			[
    ...
    valor_min = 1
    details = (
        f"<i>⚠️ Não envie um valor</i> <b>Menor</b> <i>que R${valor_min}, pois se você enviar perderá seu dinheiro.</i>"
        if valor_min
        else ""
    )
    await m.edit_message_text(
...
<b>Para Efetuar Um Pagamento, Envie O Valor Desejado Na Chave Pix Abaixo!</b>
...
Após enviar, chame @{support_user} e mande o link da transação. Será adicionado exatamente o valor que chegar à carteira.
{details}""",
        reply_markup=kb,
    )        
...
- Gift card: <code>{gift}</code>"""
    ...
    kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="💳 Compre as melhores CCs",url="https://t.me/NuckyStoreBot"
                            ),
                        ],
                    ]
                )
    mention = m.from_user.first_name
    base = f"""<a href=''>&#8204</a>🎁 {mention} resgatou um gift card de <b>R${value}</b>
