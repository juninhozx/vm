...
def call_function(name):
    # Verifica se já passou 1 minuto desde a última vez que a função foi chamada
    cur.execute("SELECT timestamp FROM last_call WHERE name=?", (name,))
    last_call = cur.fetchone()
    if last_call is not None:
        elapsed_time = time.time() - last_call[0]
        if elapsed_time < 60:
            #print("A função já foi chamada há menos de 1 minuto. Não é possível chamá-la novamente.")
            return True
    ...
    # Atualiza o registro da hora da última vez que a função foi chamada
    cur.execute("REPLACE INTO last_call (name, timestamp) VALUES (?, ?)", (name, time.time()))
    save()
...
async def verify_pay(objeto, c: Client, send, value: float):
        ...
        if rt == "PAGO":
           ## rt = "PAGO"
            ...
            await c.send_message(
                ADMIN_CHAT,
                f"<a href='https://t.me/NuckyStoreBot>&#8204</a>💰 {mention} Adicionou <b>R${value}</b> de saldo\n\n<a href='https://t.me/LittleNucky>SUPORTE</a>",
            )
            mention = user.first_name
            await c.send_message(
                GRUPO_PUB,
                f"<a href='https://t.me/NuckyStoreBot>&#8204</a>💰 {mention} Adicionou <b>R${value}</b> de saldo no bot\n\n<a href='https://t.me/LittleNucky>Suporte</a>",
            )
            save()
            return
    ...
    await send.edit_text(
        "<b>⚠️ Nenhum depósito foi feito em 5 minutos, adição de saldo cancelada.</b>",
        reply_markup=kb,
    )
...
async def auto_pay(c: Client, m: Union[CallbackQuery, Message]):
    ...
    id_flood = m.from_user.id

    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Menu Principal", callback_data="add_saldo")]
        ]
    )
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="00:00:60", callback_data="add_saldo_auto"
                )
            ]
        ]
    )
    
    if (call_function(f"{id_flood}") == True):
            return await m.reply_text(
                f"<b>⚠️ Aguarde um minuto para usar o /pix novamente!</b>", reply_markup=kb
            )
    ...
    if isinstance(m, CallbackQuery):
        ...
        cob = await m.message.ask(
            f"""<b>💎 Digite o valor que você quer depositar (mínimo R$ {min_value})!</b>""",
    ...
    cpf, full_name, email = "16026933824", "VALDEMAR LUSTROSO GONCALVES","valdemar@gmail.com"
...
⚠️ Pix automático está temporariamente em manutenção.
Caso o pagamento não seja confirmado automaticamente, envie o comprovante ao suporte: @jovensuporte
"""
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔕 já fiz o pagamento", callback_data="add_saldo"
                )
            ]
        ]
    )
