...
async def on_add_m(c: Client, m: Message):
    ...
    if cards:
        ...
        if not total:
            text = (
                "❌ Não encontrei CONTAS STREMING na sua mensagem. Envie elas como texto ou arquivo."
            )
        else:
            text = f"✅ {success} CONTAS STREMING adicionadas com sucesso. Repetidas/Inválidas: {(total - success)}"
    ...
    await m.reply_text(
        """💳 Modo de adição ativo. Envie as CONTAS STREMING como texto ou arquivo e elas serão adicionadas.
TIPO|EMAIL|SENHA|CIDADE

EXEMPLO: /contas NETFLIX|email@gmail.com|yeuue|SaoPaulo""",
    ...
    while True:
        if not first:
            await m.reply_text(
                "✅ Envie mais CONTAS STREMING ou digite /done para sair do modo de adição.",
                reply_markup=ForceReply(),
            )
    ...
    await m.reply_text(
        "✅ Você Saiu do modo de adição de CONTAS STREMING.", reply_markup=ReplyKeyboardRemove()
    )
