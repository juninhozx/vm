...
@Client.on_callback_query(filters.regex(r"^change_prices2$") & filters.user(ADMINS))
async def change_prices2(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton("⬅️ Menu Principal", callback_data="change_prices")],
        ]
    )
...
@Client.on_callback_query(
    filters.regex(r"^change_price2 (?P<ptype>.+)") & filters.user(ADMINS)
)
async def change_price(c: Client, m: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton("✅ Ok", callback_data="change_prices2")],
        ]
    )
