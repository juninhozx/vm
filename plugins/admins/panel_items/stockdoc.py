...
@Client.on_callback_query(
    filters.regex(r"^stockdoc (?P<type_name>\w+)$") & filters.user(ADMINS)
)
@Client.on_message(filters.command(["estoquedoc", "stockdoc"]))
async def stockdoc(c: Client, m: Union[CallbackQuery, Message]):
    keys = keys1.copy()
        ...
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    ...
                    callback_data="downloaddoc " + table_name,
                ),
                InlineKeyboardButton(
                    text=f"⛔️ Apagar {keys[table_name].split(maxsplit=1)[1].lower()}",
                    callback_data="cleardoc " + table_name,
                ),
            ],
            [
                InlineKeyboardButton("⬅️ Menu Principal", callback_data="stockg"),
            ],
        ]
    )
    ...
    stock = (
        ...
        or "<b>Nenhum item nesta categoria DOCS.</b>"
    )
    total = f"\n\n<b>Total</b>: {sum([int(x[1]) for x in ccs])}" if ccs else ""

    await send(
        f"<b>📨 Estoque de documentos - {keys[table_name].split(maxsplit=1)[1]}</b>\n\n{stock}{total}",
        reply_markup=kb if m.from_user.id in ADMINS else None,
    )


@Client.on_callback_query(
    filters.regex(r"^downloaddoc (?P<table>\w+)") & filters.user(ADMINS)
)
async def get_stockdoc(c: Client, m: CallbackQuery):
    table_name = m.matches[0]["table"]

    # Tables para enviar por categorias
    if table_name == "docscnh":
        tables = "nome, cpf, linkdoc, level, score, localidade, idcpf, added_date"
    elif table_name == "docs_sold":
        tables = "nome, cpf, linkdoc, level, score, localidade, idcpf, added_date, bought_date, owner"
...
@Client.on_callback_query(
    filters.regex(r"^cleardoc (?P<table>\w+)") & filters.user(ADMINS)
)
async def cleardoc_table(c: Client, m: CallbackQuery):
    table_name = m.matches[0]["table"]
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    ...
                    callback_data="cleardoc_confirm " + table_name,
                ),
                InlineKeyboardButton(
                    text="⬅️ Menu Principal",
                    callback_data="stockdoc " + table_name,
                ),
            ],
        ]
    )
...
@Client.on_callback_query(
    filters.regex(r"^cleardoc_confirm (?P<table>\w+)") & filters.user(ADMINS)
)
async def cleardoc_table_confirm(c: Client, m: CallbackQuery):
    table_name = m.matches[0]["table"]
