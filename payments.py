...
class MercadoPago:
    ...
    async def create_payment(
        ...
        rt = {
            ...
            "copy_paste": rjson.get("point_of_interaction", {}).get("transaction_data", {}).get("qr_code"),  # Use default values if missing
            "qr_code": rjson.get("point_of_interaction", {}).get("transaction_data", {}).get("qr_code_base64"),  # Use default values if missing
            "status": rjson.get("status"),
        }
        print(f"Reponse {rt}")
        print(f"Response da api {rjson}")
        return rt
